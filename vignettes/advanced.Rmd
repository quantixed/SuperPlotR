...
The wrapper approach remains the easiest way to make a SuperPlot quickly.
For more control, SuperPlotR now also supports a specification workflow:

1. Create a plot specification with `superplot_spec()`
2. Modify advanced options with `sp_modify()`
3. Render with `autoplot()`
...
This is useful when you want to tune individual layers without adding many
top-level arguments to `superplot()`.
...
You can pass the same options directly from `superplot()` using the
`options` argument:

```{r options_in_wrapper}
superplot(
  lord_jcb,
  "Speed",
  "Treatment",
...
Here's a final example of tweaking parameters for the crossbar, legend, raw points and summary points.

```{r options_in_wrapper2}
superplot(
  lord_jcb,
  meas = "Speed",
  cond = "Treatment",
