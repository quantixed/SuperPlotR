...
If we fail to supply the variable for faceting in the `superplot()` call, then when the facet function is called subsequently, the summary points and bars will be incorrect because they are calculated without the facet variable.
...
**Note:** currently `facet_wrap()` is used as the option in `superplot()`, for further enhancement requests, raise an issue.
